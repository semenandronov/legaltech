...
export const DocumentChat: React.FC<DocumentChatProps> = ({
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await chatOverDocument(documentId, inputValue.trim())
    ...
    <div className="flex flex-col h-full border-r bg-background">
      ...
      <div className="border-b p-3 bg-muted/30">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-sm">Ассистент по документу</span>
        </div>
        {documentTitle && (
        ...
        <div className="space-y-4">
          {messages.length === 0 && (
            <div className="text-center text-muted-foreground text-sm py-8">
              ...
              <p className="text-xs mt-2">Например: "Проверь документ на риски" или "Улучши формулировки"</p>
            </div>
          )}
            ...
            <div className="flex justify-start">
              <Card className="bg-muted">
                <CardContent className="p-3">
                  <div className="flex items-center gap-2">
                    <Spinner size="sm" />
                    <span className="text-sm text-muted-foreground">Анализирую документ...</span>
                  </div>
                </CardContent>
              </Card>
            </div>
