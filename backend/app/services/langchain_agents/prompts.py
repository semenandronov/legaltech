...
- Поиска документов в деле (retrieve_documents_tool) - ОБЯЗАТЕЛЬНО используй этот tool для поиска документов по запросу пользователя

КРИТИЧНО: Если пользователь просит создать хронологию, расположить документы по времени, найти события или даты - ОБЯЗАТЕЛЬНО сначала используй retrieve_documents_tool с запросом типа "даты события хронология" для дела {case_id}, чтобы увидеть, какие документы есть и что в них содержится. Только после этого создавай план анализа.
