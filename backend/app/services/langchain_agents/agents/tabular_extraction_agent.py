"""
TabularExtractionAgent - агент для извлечения структурированных данных из документов в таблицу.

# РОЛЬ
Агент-экстрактор, специализирующийся на извлечении данных из юридических документов
в табличную структуру с оценкой уверенности и поддержкой Human-in-the-Loop.

# ПАТТЕРН: Map-Reduce
1. Map: Параллельное извлечение данных из каждого документа
2. Reduce: Объединение и валидация результатов
3. HITL: Запрос уточнений для ячеек с низкой уверенностью

# КОГДА ИСПОЛЬЗОВАТЬ
- Нужно извлечь структурированные данные из множества документов
- Требуется заполнить таблицу с предопределёнными колонками
- Важна точность и подтверждение источников

# КОГДА НЕ ИСПОЛЬЗОВАТЬ
- Простой поиск информации → используй rag_search
- Нужен анализ или выводы → используй ChatReActAgent
...
class TabularExtractionAgent:
    ...
    def _extract_cell_value(
        ...
        """
        # Формируем промпт на основе типа колонки с подробными инструкциями
        type_instructions = {
            "text": """Извлеки текстовое значение.
                ФОРМАТ: Строка текста без кавычек.
                ПРИМЕРЫ: "ООО Ромашка", "Иванов Иван Иванович", "г. Москва, ул. Ленина, д. 1"
                ЕСЛИ НЕ НАЙДЕНО: верни null""",
            "number": """Извлеки числовое значение.
                ФОРМАТ: Только число без единиц измерения.
                ПРИМЕРЫ: 100, 1500000, 3.14
                ЕСЛИ НЕ НАЙДЕНО: верни null""",
            "currency": """Извлеки денежную сумму.
                ФОРМАТ: Число с валютой (руб., USD, EUR).
                ПРИМЕРЫ: "1500000 руб.", "50000 USD", "25000 EUR"
                ЕСЛИ НЕ НАЙДЕНО: верни null""",
            "yes_no": """Определи, присутствует ли условие/факт.
                ФОРМАТ: Только "Да" или "Нет" (без кавычек).
                ЛОГИКА: "Да" если факт явно подтверждён, "Нет" если явно отрицается.
                ЕСЛИ НЕЯСНО: верни null с низкой уверенностью""",
            "date": """Извлеки дату.
                ФОРМАТ: ДД.ММ.ГГГГ (например, 15.03.2024).
                ПРИМЕРЫ: "01.01.2024", "31.12.2023"
                ЕСЛИ НЕ НАЙДЕНО: верни null""",
            "tag": """Выбери подходящий тег из списка.
                ФОРМАТ: Точное значение из списка доступных тегов.
                ЛОГИКА: Выбери наиболее подходящий тег на основе контекста.
                ЕСЛИ НЕ ПОДХОДИТ НИ ОДИН: верни null""",
            "verbatim": """Процитируй точный текст из документа.
                ФОРМАТ: Дословная цитата в кавычках.
                ЛОГИКА: Копируй текст без изменений, включая пунктуацию.
                ЕСЛИ НЕ НАЙДЕНО: верни null"""
        }
        ...
        if column.column_type == "tag" and column.config:
            ...
            if options:
                ...
                tag_options = f"\nДОСТУПНЫЕ ТЕГИ (выбери один): {', '.join(tag_labels)}"
        
        prompt = f"""# РОЛЬ
Ты — точный экстрактор данных из юридических документов.

# ЦЕЛЬ
Извлечь конкретное значение из документа и подтвердить его цитатой.
...
2. Найди информацию, соответствующую задаче
3. Извлеки значение в указанном формате
4. Найди цитату, подтверждающую значение
5. Оцени уверенность (0.0-1.0):
   - 1.0: Значение явно указано, цитата точная
   - 0.7-0.9: Значение найдено, но требует интерпретации
   - 0.4-0.6: Значение предположительное
   - 0.0-0.3: Информация не найдена или очень неточная

# ОГРАНИЧЕНИЯ (GUARDRAILS)
- НЕ выдумывай значения, которых нет в документе
- НЕ интерпретируй данные сверх того, что написано
- НЕ путай похожие значения (даты, суммы разных сторон)
- ВСЕГДА указывай источник (цитату)

# ФОРМАТ ОТВЕТА (JSON)
{{
    "value": "извлечённое значение или null",
    "confidence": 0.0-1.0,
    "source_quote": "точная цитата из документа",
    "source_page": номер_страницы или null
}}
...
{{
    "value": "1500000 руб.",
    "confidence": 0.95,
    "source_quote": "Цена договора составляет 1 500 000 (один миллион пятьсот тысяч) рублей",
    "source_page": 2
}}

# ПРИМЕР КОГДА НЕ НАЙДЕНО
{{
    "value": null,
    "confidence": 0.0,
    "source_quote": null,
