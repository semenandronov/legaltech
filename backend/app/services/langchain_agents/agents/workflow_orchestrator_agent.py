"""
WorkflowOrchestratorAgent - агент-оркестратор для выполнения сложных многошаговых workflows.

# РОЛЬ
Координатор выполнения сложных юридических задач, состоящих из множества шагов.
Планирует, оптимизирует и контролирует выполнение workflow с поддержкой HITL.

# ПАТТЕРН: Планирование + Параллельное выполнение
1. Анализ: Определение структуры и зависимостей workflow
2. Планирование: Генерация оптимального плана с оценкой времени
3. HITL: Запрос одобрения плана у пользователя (опционально)
4. Выполнение: Параллельное выполнение независимых шагов
5. Мониторинг: Отслеживание прогресса и адаптация при ошибках
6. Синтез: Объединение результатов всех шагов

# КОГДА ИСПОЛЬЗОВАТЬ
- Сложная задача с 3+ зависимыми шагами
- Требуется координация нескольких типов анализа
- Нужна автоматическая адаптация при ошибках
- Важен контроль пользователя над планом выполнения

# КОГДА НЕ ИСПОЛЬЗОВАТЬ
- Простой вопрос-ответ → используй ChatReActAgent
- Извлечение данных в таблицу → используй TabularExtractionAgent
- Одношаговая операция → используй соответствующий узел напрямую

# ТИПЫ ШАГОВ
- analysis: Анализ документов (риски, противоречия, факты)
- extraction: Извлечение структурированных данных
- generation: Генерация документов
- review: Проверка качества результатов
- custom: Кастомная логика через handler
"""
...
class WorkflowOrchestratorAgent:
    ...
    async def _execute_analysis_step(
        ...
        # Формируем промпт по best practices
        prompt = f"""# РОЛЬ
Ты — юридический аналитик, выполняющий шаг анализа в рамках workflow.

# ЦЕЛЬ
Провести анализ "{step.name}" и предоставить структурированные результаты.

# ЗАДАЧА
{step.description}

# КОНТЕКСТ ИЗ ДОКУМЕНТОВ ДЕЛА
{context if context else "Документы не предоставлены"}

# РЕЗУЛЬТАТЫ ПРЕДЫДУЩИХ ШАГОВ WORKFLOW
{json.dumps(dependency_results, ensure_ascii=False, indent=2) if dependency_results else "Это первый шаг"}

# ИНСТРУКЦИИ
1. Внимательно изучи контекст из документов
2. Учти результаты предыдущих шагов
3. Проведи анализ согласно описанию задачи
4. Сформулируй конкретные находки
5. Дай практические рекомендации

# ОГРАНИЧЕНИЯ (GUARDRAILS)
- НЕ выдумывай факты, которых нет в документах
- НЕ игнорируй результаты предыдущих шагов
- ВСЕГДА указывай источник для каждой находки
- Давай КОНКРЕТНЫЕ, а не общие рекомендации

# ФОРМАТ ОТВЕТА (JSON)
{{
    "findings": [
        "Находка 1: конкретный факт с указанием источника",
        "Находка 2: ещё один факт"
    ],
    "summary": "Краткое резюме анализа в 2-3 предложениях",
    "recommendations": [
        "Рекомендация 1: конкретное действие",
        "Рекомендация 2: ещё одно действие"
    ],
    "risks": ["Риск 1", "Риск 2"],
    "confidence": 0.0-1.0
}}
"""
    ...
    async def _execute_extraction_step(
        ...
        if extraction_type == "tabular":
            ...
            # Общее извлечение с улучшенным промптом
            prompt = f"""# РОЛЬ
Ты — экстрактор данных, выполняющий шаг извлечения в рамках workflow.

# ЦЕЛЬ
Извлечь структурированные данные для шага "{step.name}".
...
# ИНСТРУКЦИИ
1. Определи, какие данные нужно извлечь
2. Найди эти данные в контексте или предыдущих шагах
3. Структурируй результат в JSON

# ОГРАНИЧЕНИЯ (GUARDRAILS)
- НЕ выдумывай данные
- Извлекай ТОЛЬКО то, что явно присутствует
- Для отсутствующих данных используй null

# ФОРМАТ ОТВЕТА (JSON)
{{
    "extracted_data": {{}},
    "sources": ["источник 1"],
    "missing_fields": ["поле, которое не найдено"],
    "confidence": 0.0-1.0
}}
"""
    ...
    async def _execute_generation_step(
        ...
        prompt = f"""# РОЛЬ
Ты — профессиональный юрист, создающий юридический документ.

# ЦЕЛЬ
Создать документ "{step.name}" на основе собранных данных.

# ЗАДАЧА
{step.description}

# ДАННЫЕ ДЛЯ ДОКУМЕНТА (из предыдущих шагов)
{json.dumps(dependency_results, ensure_ascii=False, indent=2) if dependency_results else "Данные не предоставлены"}

# ИНСТРУКЦИИ
1. Используй стандартную структуру для данного типа документа
2. Заполни все обязательные поля из предоставленных данных
3. Отметь [ЗАПОЛНИТЬ] для данных, которых не хватает
4. Используй профессиональный юридический язык
5. Форматируй в HTML для редактора

# ОГРАНИЧЕНИЯ (GUARDRAILS)
- НЕ выдумывай реквизиты, даты, суммы
- НЕ пропускай обязательные разделы
- Отмечай места для заполнения пользователем
- Следуй стандартам оформления юридических документов

# ФОРМАТ
Сгенерируй документ в формате HTML с правильной структурой.
"""
    ...
    async def _execute_review_step(
        ...
        """Выполнить шаг ревью."""
        prompt = f"""# РОЛЬ
Ты — контролёр качества, проверяющий результаты работы workflow.

# ЦЕЛЬ
Провести ревью "{step.name}" и оценить качество результатов.
...
# КРИТЕРИИ ОЦЕНКИ
1. Полнота: Все ли требуемые данные присутствуют?
2. Точность: Соответствуют ли данные источникам?
3. Консистентность: Нет ли противоречий между шагами?
4. Качество: Соответствует ли результат профессиональным стандартам?

# ИНСТРУКЦИИ
1. Проверь каждый результат по критериям
2. Выяви проблемы и несоответствия
3. Оцени общее качество (0-100)
4. Дай конкретные рекомендации по улучшению
5. Прими решение: одобрить или отклонить

# ОГРАНИЧЕНИЯ (GUARDRAILS)
- НЕ игнорируй очевидные ошибки
- НЕ одобряй результаты с критическими проблемами
- Будь объективным в оценке

# ФОРМАТ ОТВЕТА (JSON)
{{
    "quality_score": 0-100,
    "issues": [
        {{"severity": "critical|major|minor", "description": "описание проблемы"}}
    ],
    "recommendations": ["конкретная рекомендация"],
    "approved": true/false,
    "approval_reason": "причина решения"
}}
"""
    ...
    async def _execute_custom_step(
        ...
        # Fallback к LLM с улучшенным промптом
        prompt = f"""# РОЛЬ
Ты — универсальный исполнитель задач в рамках юридического workflow.

# ЦЕЛЬ
Выполнить кастомный шаг "{step.name}".
...
# ИНСТРУКЦИИ
1. Внимательно изучи описание задачи
2. Используй конфигурацию для настройки выполнения
3. Учти результаты предыдущих шагов
4. Выполни задачу и верни структурированный результат

# ОГРАНИЧЕНИЯ (GUARDRAILS)
- Выполняй ТОЛЬКО то, что описано в задаче
- НЕ выходи за рамки конфигурации
...
{{
    ...
    "result": {{}},
    "notes": ["примечание"],
    "next_steps": ["рекомендуемое действие"]
}}
"""
